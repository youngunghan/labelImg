...
- **로드 시 모든 박스가 `difficult=True`로 들어온다**(`libs/create_ml_io.py:133`) — 코드상 하드코딩이라 다른 포맷과 동작이 다르다. 학습 전처리에서 difficult를 거른다면 유의.
- Reader는 JSON 안에서 `"image"`가 현재 이미지 **basename과 일치하는 항목**의 `verified`를 읽는다(`output_list[0]`이 아니다) — 다중 이미지 JSON에서도 해당 이미지의 verified 플래그를 정확히 찾는다(`libs/create_ml_io.py:114-123`).
