...
- 기본 설치 상태(`pyqt5`+`lxml`만, `[ai]` extras도 설정도 없음)에서는 `Ctrl+I`를 눌러도 아무 일도
  ...
  configured" 안내가 뜬다(`NO_BACKEND_CONFIGURED_HINT`, `libs/assist/controller.py:73-76`,
  `_unavailable_hint`/`refresh_actions`, `libs/assist/controller.py:155-166, 242-279`).
- **AI 메뉴를 켜려면** [위 절차](#내-onnx-모델을-앱에-연결하기)대로 `model/backend`를 `yolo_onnx`로,
  ...
  (`BACKEND_UNAVAILABLE_HINT`, `libs/assist/controller.py:77-80`)가 뜬다.
...
- 예측은 UI 스레드를 막지 않는 별도 워커에서 돈다(`InferenceService`, 단일 워커 QThreadPool). 추론 중에
  ...
  다르면 조용히 버려진다**(`AssistController._is_current`, `libs/assist/controller.py:363-372`) — 새
