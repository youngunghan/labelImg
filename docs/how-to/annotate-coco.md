...
**고정된 데이터셋 json 하나**로 취급한다(`libs/coco_io.py:11-17`, `labelImg.py:676-693`).
...
- **기본 대상**: `<저장 폴더>/annotations.json` — `Change Save Dir`로 지정한 저장 폴더(없으면 이미지가 있는
  ...
  (`coco_dataset_target`, `labelImg.py:676-693`).
- **File → Import COCO...** / **File → Export COCO...**(`labelImg.py:257-260`, 단축키 없음)로 다른 json을
  ...
  (`self.coco_dataset_path`, `labelImg.py:2342`·`2367`).
...
일어난다(`save_labels`의 COCO 분기, `labelImg.py:1072-1083`; `save_file`의 COCO 선분기,
`labelImg.py:1720-1726`):
...
(데이터셋 json)** 이다(`show_bounding_box_from_annotation_file`, `labelImg.py:1356-1393`) — 즉 저장 폴더에
...
- **`difficult`와 `verified`는 COCO 스키마에 저장 슬롯이 없다.** 두 플래그 모두 저장 시 버려지고, 다시
  ...
  뜬다(`labelImg.py:1638-1644`) — 다음 로드에서 조용히 사라지는 대신 그 자리에서 알려준다.
- **이미지 분류(Good/Bad, `g`/`b`)로 이미지를 옮기면 COCO 데이터셋의 항목은 그대로 남는다.** COCO는
  ...
  (`labelImg.py:1940-1944`). 즉 이미지를 옮긴 뒤에는 데이터셋 json에 더 이상 존재하지 않는 경로를 가리키는
