...
`make clean`도 이 파일을 지운다. 내부적으로 `Reset All`은 `Settings.reset()`(파일 삭제 + 메모리 초기화) 후 창을 닫고 `QProcess.startDetached`로 앱을 재시작한다(`labelImg.py:1853-1857`). 단, 재시작은 `os.path.abspath(__file__)`(파이썬 스크립트 경로)를 직접 실행하는 방식이라 Windows나 빌드된 exe에서는 자동 재시작이 안 될 수 있다 — 그 경우 창이 닫힌 뒤 수동으로 다시 실행하면 된다(설정 초기화 자체는 완료된 상태).
...
클래스 목록은 설정 pkl이 아니라 `predefined_classes.txt`에서 읽는다(소스 실행: `data/predefined_classes.txt`, exe 실행: exe 옆 `predefined_classes.txt` — `get_persistent_classes_file`, `labelImg.py:1939`). pkl을 지우거나 `Reset All`을 해도 클래스는 바뀌지 않는다. 메뉴 `File → Edit Default Classes`(Ctrl+Shift+E)로 수정하거나 해당 txt 파일을 직접 편집하라(한 줄에 한 클래스).
...
라벨 파일이 멀쩡한데도 박스가 안 뜨면 `default_save_dir`가 다른 폴더를 가리키는지 의심하라. `default_save_dir`가 설정돼 있으면 라벨은 그 폴더에서**만** 찾고, 설정이 없을 때만 이미지 옆에서 찾는다(`labelImg.py:1220-1247`). 설정 pkl에 남은 이전 세션의 저장 폴더가 시작 시 `default_save_dir`로 복원되므로(`labelImg.py:532-535`) 이것이 원인일 수 있다. `Change Save Dir`로 이미지 폴더를 다시 지정하거나 위의 설정 리셋을 하라. 참고: 명령줄 인자로 이미지 폴더를 넘겨도 pkl의 마지막 폴더가 우선돼 라벨을 못 찾던 버그는 소스(`python labelImg.py`)와 2026-07-03 재빌드된 `dist\labelImg.exe`에서는 수정됐다(`open_dir_dialog`, `labelImg.py:1383-1414`). 구버전 exe(루트 `dist_labelimg.zip`에 포함, 2026-06-26 빌드; zip 파일 자체의 최종 수정 시각은 06-29)에는 이 버그가 남아 있다 — 그 exe를 압축 해제해 쓸 때는 pkl 삭제가 우회책이다.

### Open으로 단일 파일을 열었더니 File List가 사라짐

`load_file`은 열려는 파일이 현재 이미지 목록(`m_img_list`)에 없으면 File List를 비운다(`labelImg.py:1152-1154`). 폴더 단위로 작업하려면 `Open Dir`을 쓰라.

### 한글(비ASCII) 클래스명이 깨진다

(2026-07-08 해결) YOLO 라벨 txt·`classes.txt`·CreateML JSON 모두 읽기/쓰기가 UTF-8로 고정되어 비ASCII 클래스명이 플랫폼과 무관하게 라운드트립된다. 그 이전 버전으로 저장된 파일이 로케일 인코딩(cp949 등)으로 남아 있으면 UTF-8로 다시 저장해야 한다.
