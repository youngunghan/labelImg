...
3. 다른 폴더에 저장하려면 **폴더를 연 뒤에** `File → Change Save Dir`(`Ctrl+r`)로 저장 폴더를 지정한다. ⚠️ 메뉴의 Open Dir는 저장 폴더를 연 폴더로 덮어쓰므로(`open_dir_dialog` 말미, `labelImg.py:1397`), Change Save Dir를 Open Dir보다 먼저 하면 지정이 소리 없이 무효가 된다. (명령줄 3번째 인자 `save_dir`로 시작한 경우에는 시작 시점에 그 폴더가 저장 폴더로 유지된다.)
...
단, `Change Save Dir`로 저장 폴더를 바꾼 상태라면 XML은 이미지 옆이 아니라 **저장 폴더**에서 찾는다(`labelImg.py:1204-1219`). 같은 이름의 `.xml` > `.txt`(YOLO) > `.json`(CreateML) 순으로 탐색된다.
