...
- **`StubBackend`가 테스트를 구동한다.** 모델 파일도, `onnxruntime`도 없이 어시스트 로직(수락/거부, provisional 필터, stale 폐기)을 결정론적으로 검증한다 — `tests/test_inference_core.py`, `tests/test_assist.py`. 테스트는 30개에서 **229개**(테스트 파일 13개)로 늘었고 코어 매트릭스는 AI 의존성 없이 green이다.
...
셋 다 됐다: **지연 import와 스텁 기반 코어 테스트는 Phase 1**부터(기본 의존성은 여전히 `pyqt5`+`lxml`뿐이고, 코어 테스트 229개는 numpy조차 없이 돈다), **`[ai]` extra(`onnxruntime`·`numpy`)는 Phase 2**에서 추가됐다(`setup.py:27`).
