...
`Ctrl` 키를 누르고 있으면(MainWindow의 `keyPressEvent`/`keyReleaseEvent`가 `set_drawing_shape_to_square` 토글, `labelImg.py:622-629`) 그리기 중 `line[1]`이 시작점 기준 정사각형 좌표로 강제된다(`canvas.py:148-155`).
