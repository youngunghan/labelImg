...
구현: `libs/assist/controller.py`(`AssistController`). 이미지가 열려 있고 사용 가능한 모델 백엔드가 있을 때만 활성화된다(`refresh_actions`, `controller.py:242-279`) — 백엔드가 없으면(base install 등) 툴팁에 설치 안내가 뜬다.
