...
| `libs/assist/controller.py` (1568줄) | `AssistController`(QObject, `:173`) | MainWindow가 AI를 위임하는 유일한 객체. 액션: **Auto-label Image=Ctrl+I**(`:508-511`)·**Accept All=Ctrl+Return**(`:512-515`)·**Reject All=Ctrl+Backspace**(`:516-519`)+신뢰도 슬라이더(`_create_threshold_action`, `:551-583`). `on_prediction_ready`(`:810`)가 `_is_current`(`:873`)로 **stale 결과를 드롭**(이미지 전환 후 늦게 도착한 예측이 엉뚱한 이미지에 박히는 것 방지) · `provisional_shapes()`(`:886`)는 `_shapes` 캐시가 아니라 **캔버스를 직접 읽음**(Ctrl+D 복제나 수동 삭제로 캔버스와 내부 상태가 어긋날 수 있다는 주석, `:886-906`) · `accept_all`/`reject_all`(`:964`/`:985`). **설정 읽기 시 레거시 `'stub'` 무시**: `__init__`(`:175-194`)이 `SETTING_MODEL_BACKEND`가 `'stub'`이면(구 `DEFAULT_BACKEND`가 `'stub'`이던 시절 저장된 값) 미설정으로 취급한다(`_LEGACY_IMPLICIT_DEFAULT_BACKEND`, `:95-110`) → [settings.md](settings.md). **모델 스왑 시 이전 상태 정리**: `set_backend`(`:324-347`)가 백엔드를 바꿀 때마다 `clear_suggestions()`를 호출해 이전 모델의 provisional 박스·`_dismissed` 추적을 지우고, `_interactive_generation`을 태깅해 스왑 전에 디스패치된 대화형(Ctrl+I) 요청이 스왑 후에 응답해도 드롭한다(`auto_label_image`, `:720-747`; `on_prediction_ready`, `:810-845`; `on_prediction_failed`, `:847-871` — 교차엔진 리뷰로 발견, 배치가 이미 쓰던 `_batch_generation` 태깅과 같은 원칙). **능동학습 (Phase 4)**: `score_folder`/`cancel_batch_scoring`(`:1065-1139`)가 `m_img_list`를 한 번에 한 이미지씩 배치 채점하고(로드 실패가 연속될 때 재귀로 스택이 자라지 않도록 `_batch_step`의 실패 분기는 `QTimer.singleShot(0, ...)`로 다음 스텝을 이벤트 루프에 미룬다, `_advance_batch`의 `synchronous`, `:1141-1215` — 백엔드가 배치 도중 사라지는 동형 재귀는 `InferenceService._emit_failed_deferred`(위 InferenceService 행 참고)가 emit 쪽에서 막는다), `on_prediction_ready`/`on_prediction_failed`가 각 요청을 디스패치 순서로 태깅해(`_dispatch_request`/`_pop_request_kind`, `:749-808`) 그 결과를 대화형 stale-drop과 분리해서 처리한다(경로 일치만으로 구분하던 이전 방식은 배치 시작 시점에 아직 안 끝난 대화형 요청과 경로가 겹치면 뒤바뀌는 레이스가 있었다 — 교차엔진 리뷰로 발견, 수정됨) · Score Folder 액션은 배치 실행 중이면 `m_img_list`가 비거나 `available`이 `False`가 돼도(예: 배치 도중 Model Settings에서 AI를 꺼도) 켜져 있다(취소 가능하도록, `refresh_actions`의 `batch_running`, `:642-652` — `apply_model_settings`의 비활성화 분기도 `cancel_batch_scoring()`을 먼저 호출해 이중으로 막는다) · `sort_by_uncertainty`/`restore_original_order`/`_reorder`(`:1267-1389`)가 `m_img_list`를 재정렬하고 현재 선택을 보존한다 · Open Directory 직후에도 열린 이미지의 행이 계속 선택 표시되도록 위젯을 다시 채운 뒤 재선택한다(`refresh_file_list`, `:1434-1481`) · `_ranks()`/표시되는 "채점 N" 총계는 `m_img_list`에 아직 있는 이미지만 센다(분류로 폴더를 떠난 이미지의 점수는 undo를 위해 `_uncertainty`엔 남지만 순위엔서 제외, `:1515-1542`) |
| `libs/assist/settings_dialog.py` (128줄) | `ModelSettingsDialog`(QDialog, `:31`) | "Model Settings..." 메뉴 액션(`AssistController.open_model_settings_dialog`, `controller.py:457-470`)이 여는 다이얼로그. 백엔드 선택(`BACKEND_CHOICES`, `사용 안 함`/`YOLO (ONNX)` -- `stub`은 의도적으로 미노출, `:43-46`)과 `.onnx` 경로 입력 + Browse(`QFileDialog.getOpenFileName`, `:103-104`; PyQt4 폴백의 bare-string 반환은 `isinstance` 가드가 튜플 언패킹보다 먼저 실행돼야 한다 -- 교차엔진 리뷰로 발견, `_browse`, `:103-118`)만 모으는 얇은 셸 -- 검증·영속화·백엔드 재구성은 전부 `AssistController.apply_model_settings`(`controller.py:349-455`)에 있고, 그 메서드는 `exec_()` 없이 직접 호출해 테스트한다(`tests/test_model_settings.py`) |
