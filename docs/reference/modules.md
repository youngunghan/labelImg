...
| `libs/assist/controller.py` (1249줄) | `AssistController`(QObject, `:112`) | MainWindow가 AI를 위임하는 유일한 객체. 액션: **Auto-label Image=Ctrl+I**(`:267-270`)·**Accept All=Ctrl+Return**(`:271-274`)·**Reject All=Ctrl+Backspace**(`:275-278`)+신뢰도 슬라이더(`_create_threshold_action`, `:307-339`). `on_prediction_ready`(`:535`)가 `_is_current`(`:573`)로 **stale 결과를 드롭**(이미지 전환 후 늦게 도착한 예측이 엉뚱한 이미지에 박히는 것 방지) · `provisional_shapes()`(`:586`)는 `_shapes` 캐시가 아니라 **캔버스를 직접 읽음**(Ctrl+D 복제나 수동 삭제로 캔버스와 내부 상태가 어긋날 수 있다는 주석, `:586-606`) · `accept_all`/`reject_all`(`:664`/`:685`). **설정 읽기 시 레거시 `'stub'` 무시**: `__init__`(`:114-133`)이 `SETTING_MODEL_BACKEND`가 `'stub'`이면(구 `DEFAULT_BACKEND`가 `'stub'`이던 시절 저장된 값) 미설정으로 취급한다(`_LEGACY_IMPLICIT_DEFAULT_BACKEND`, `:94-109`) → [settings.md](settings.md). **능동학습 (Phase 4)**: `score_folder`/`cancel_batch_scoring`(`:765-827`)가 `m_img_list`를 한 번에 한 이미지씩 배치 채점하고(로드 실패가 연속될 때 재귀로 스택이 자라지 않도록 `_batch_step`의 실패 분기는 `QTimer.singleShot(0, ...)`로 다음 스텝을 이벤트 루프에 미룬다, `_advance_batch`의 `synchronous`, `:829-903`), `on_prediction_ready`/`on_prediction_failed`가 각 요청을 디스패치 순서로 태깅해(`_dispatch_request`/`_pop_request_kind`, `:490-533`) 그 결과를 대화형 stale-drop과 분리해서 처리한다(경로 일치만으로 구분하던 이전 방식은 배치 시작 시점에 아직 안 끝난 대화형 요청과 경로가 겹치면 뒤바뀌는 레이스가 있었다 — 교차엔진 리뷰로 발견, 수정됨) · Score Folder 액션은 배치 실행 중이면 `m_img_list`가 비어도 켜져 있다(취소 가능하도록, `refresh_actions`의 `batch_running`, `:355-426`) · `sort_by_uncertainty`/`restore_original_order`/`_reorder`(`:948-1069`)가 `m_img_list`를 재정렬하고 현재 선택을 보존한다 · Open Directory 직후에도 열린 이미지의 행이 계속 선택 표시되도록 위젯을 다시 채운 뒤 재선택한다(`refresh_file_list`, `:1115-1162`) · `_ranks()`/표시되는 "채점 N" 총계는 `m_img_list`에 아직 있는 이미지만 센다(분류로 폴더를 떠난 이미지의 점수는 undo를 위해 `_uncertainty`엔 남지만 순위엔서 제외, `:1196-1223`) |
