...
| `libs/assist/controller.py` (538줄) | `AssistController`(QObject, `:79`) | MainWindow가 AI를 위임하는 유일한 객체. 액션: **Auto-label Image=Ctrl+I**(`:173-176`)·**Accept All=Ctrl+Return**(`:177-180`)·**Reject All=Ctrl+Backspace**(`:181-184`)+신뢰도 슬라이더(`_create_threshold_action`, `:196-228`). `on_prediction_ready`(`:339`)가 `_is_current`(`:359`)로 **stale 결과를 드롭**(이미지 전환 후 늦게 도착한 예측이 엉뚱한 이미지에 박히는 것 방지) · `provisional_shapes()`(`:372`)는 `_shapes` 캐시가 아니라 **캔버스를 직접 읽음**(Ctrl+D 복제나 수동 삭제로 캔버스와 내부 상태가 어긋날 수 있다는 주석, `:372-392`) · `accept_all`/`reject_all`(`:450`/`:471`) |
